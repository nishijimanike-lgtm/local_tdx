...
impl<'a> Tdx for Kline<'a> {
    ...
    fn parse(&mut self, v: Vec<u8>) {
        ...
        // Servers may return a different bar count than requested (or a short/empty
        // payload). Prefer the wire count and skip remaining bars if the body ends early.
        ...
        for item in self.data.iter_mut() {
            ...
            pos += 4;
            // price() advances pos; guard remaining body loosely
            if pos >= v.len() {
                break;
            }
        ...
        // Drop unfilled trailing defaults if we broke early
